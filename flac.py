...
def flac_loss(protected_attr_features, features, labels, d=1):
    ...
    protected_s =  (protected_s / (torch.sum(protected_s* mask.int().float(), dim=1, keepdim=True)+ 1e-7))
    features_s = features_s / (torch.sum(features_s* mask.int().float(), dim=1, keepdim=True)+ 1e-7)

    # Jeffrey's divergence
    loss = (protected_s[mask] - features_s[mask]) * (torch.log(protected_s[mask]) - torch.log(features_s[mask]))
